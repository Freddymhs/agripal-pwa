...
export function useProjectDashboard(
  ...
  return useMemo(() => {
    ...
    const aguaEstanques = estanques.reduce(
    ...
    const diasRestantes =
      aguaNecesaria > 0
        ? aguaDisponible / (aguaNecesaria / DIAS_POR_SEMANA)
        : Infinity;
    ...
    for (const planta of plantas) {
      ...
      if (!cultivo) continue;
      plantasPorCultivo[cultivo.nombre] =
        (plantasPorCultivo[cultivo.nombre] || 0) + 1;
    }
    ...
    return {
      ...
      total_plantas: plantas.filter((p) => p.estado !== ESTADO_PLANTA.MUERTA)
        .length,
      plantas_por_cultivo: plantasPorCultivo,
      plantas_produciendo: plantas.filter(
        (p) => p.estado === ESTADO_PLANTA.PRODUCIENDO,
      ).length,
      plantas_muertas: plantas.filter((p) => p.estado === ESTADO_PLANTA.MUERTA)
        .length,
