...
export function useMapHandlers({
  ...
  const handleMapClick = useCallback(
    async (x: number, y: number) => {
      ...
      if (
        ...
        if (
          xRelativo >= 0 &&
          xRelativo <= zonaSeleccionada.ancho &&
          yRelativo >= 0 &&
  ...
  const handleCambiarEstadoPlanta = async (estado: EstadoPlanta) => {
    ...
    const result = await plantasHook.cambiarEstado(
  ...
  const handlePlantarGrid = async (espaciado: number) => {
    ...
    toast.success(
      `Se crearon ${result.plantas.length} plantas de ${cultivoSel.nombre}`,
    );
  };
  ...
  const validarCambiosZona = useCallback(
    (
      ...
      if (nuevoTam.ancho < 1 || nuevoTam.alto < 1)
        return {
          valida: false,
          error: "Las dimensiones deben ser al menos 1m × 1m",
        };
      if (nuevaPos.x + nuevoTam.ancho > terrenoActual.ancho_m)
        return {
          valida: false,
          error: "La zona excedería el ancho del terreno",
        };
      if (nuevaPos.y + nuevoTam.alto > terrenoActual.alto_m)
        return {
          valida: false,
          error: "La zona excedería el alto del terreno",
        };
      const zonaVirtual = {
        ...zonaSeleccionada,
        x: nuevaPos.x,
      ...
      for (const otraZona of zonas.filter(
        (z) => z.id !== zonaSeleccionada.id,
      )) {
  ...
  const handleCrearZona = async (data: {
    nombre: string;
    tipo: TipoZona;
  ...
  const handleMoverPlantasSeleccionadas = useCallback(
    async (plantaId: string, deltaX: number, deltaY: number) => {
      ...
      const result = await plantasHook.moverPlanta(
        plantaId,
        { x: nuevaX, y: nuevaY },
        zona,
        plantas,
