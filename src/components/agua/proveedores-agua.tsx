...
export function ProveedoresAgua({ proveedores, onChange }: ProveedoresAguaProps) {
  ...
  const handleSave = () => {
    ...
    if (nuevoProveedor) {
      ...
      onChange(proveedores.map(p => p.id === editandoId ? { ...p, ...form } : p))
    }
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <button onClick={handleAdd} className="text-sm text-green-600 hover:text-green-700 font-medium">+ Agregar</button>
      </div>
      ...
      {proveedorPrincipal && (
        <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          {proveedorPrincipal.telefono && <div className="text-sm text-gray-600">{proveedorPrincipal.telefono}</div>}
          {proveedorPrincipal.precio_m3_clp && <div className="text-sm text-gray-600">{formatCLP(proveedorPrincipal.precio_m3_clp)}/m³</div>}
        </div>
      )}
          ...
          <div key={prov.id} className="p-3 border rounded-lg bg-white">
            <div className="flex justify-between items-start">
              <div>
                ...
                {prov.precio_m3_clp && <div className="text-sm text-gray-600">{formatCLP(prov.precio_m3_clp)}/m³</div>}
                {prov.confiabilidad && (
                  <span className={`text-xs px-2 py-0.5 rounded ${
                    prov.confiabilidad === 'alta' ? 'bg-green-100 text-green-700' :
                    prov.confiabilidad === 'media' ? 'bg-yellow-100 text-yellow-700' :
                    'bg-red-100 text-red-700'
                  }`}>{prov.confiabilidad}</span>
                )}
              </div>
              <div className="flex gap-2">
                <button onClick={() => handleSetPrincipal(prov.id)} className="text-xs text-blue-600 hover:underline">Marcar principal</button>
                <button onClick={() => handleEdit(prov)} className="text-xs text-gray-600 hover:underline">Editar</button>
                <button onClick={() => handleDelete(prov.id)} className="text-xs text-red-600 hover:underline">Eliminar</button>
              </div>
            </div>
            {prov.notas && <p className="text-xs text-gray-500 mt-1">{prov.notas}</p>}
          </div>
        ))}
      </div>
      ...
      {proveedores.length === 0 && !nuevoProveedor && (
        <div className="text-center p-4 bg-gray-50 rounded-lg text-gray-500 text-sm">No hay proveedores registrados</div>
      )}
    </div>
  )
}
