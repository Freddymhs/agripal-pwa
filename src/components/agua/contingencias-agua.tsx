...
export function ContingenciasAguaPanel({
  ...
  const handleAddPlan = () => {
    if (planItem.trim() && onChange) {
      onChange({
        ...contingencias,
        plan_si_no_llega: [
          ...(contingencias?.plan_si_no_llega || []),
          planItem.trim(),
        ],
      });
      setPlanItem("");
  ...
  const proveedoresAlternativos =
    proveedores?.filter((p) => !p.es_principal) || [];
    ...
    <div className="space-y-4">
            ...
            <input
              ...
              onChange={(e) =>
                onChange?.({
                  ...contingencias,
                  alerta_critica_pct:
                    e.target.value === "" ? 20 : parseInt(e.target.value),
                })
              }
              className="w-full px-3 py-2 border rounded text-gray-900"
            />
            <p className="text-xs text-gray-500 mt-1">
              Llamar aljibe inmediatamente
            </p>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-gray-500 mt-2">
          Ejemplos: Llamar proveedor X, Reducir riego 50%, Activar reserva de
          emergencia
        </p>
      </div>
