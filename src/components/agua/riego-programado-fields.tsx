      ...
      <div>
        ...
        <div className="flex gap-2 items-center">
          <input
            ...
            value={horasDia}
            onChange={(e) =>
              onHorasDiaChange(clamp(Number(e.target.value), 1, 24))
            }
            min={1}
            max={24}
        ...
        {consumoRecomendadoLDia != null &&
          ...
          (() => {
            ...
            const resumen =
              porcentaje < 70
                ? `${porcentaje}% de lo recomendado (riego bajo).`
                : porcentaje <= 130
                  ? `${porcentaje}% de lo recomendado (casi igual).`
                  : `${porcentaje}% de lo recomendado (riego alto).`;
            return (
              <div className="bg-blue-50 border border-blue-200 rounded p-2 mt-2">
                <p className="text-xs text-blue-800">
                  Para este caudal (<strong>{caudalTotal} L/h</strong>), lo
                  recomendado es regar{" "}
                  <strong>~{horasRec.toFixed(1)} h/día</strong>.
                </p>
                <p className="text-[11px] text-blue-600 mt-0.5">
                  Con tus <strong>{horasDia} h/día</strong> actuales riegas ~
                  <strong>{Math.round(gastoActualL)} L/día</strong> ({resumen}).
                </p>
              </div>
            );
          })()}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input
            type="time"
            value={horarioFin}
            onChange={(e) => onHorarioFinChange(e.target.value)}
            className="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-blue-500"
          />
        </div>
      </div>
