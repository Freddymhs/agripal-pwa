...
export function ResumenAgua({
  ...
  const zonasConsumo = zonas
    ...
    .map(zona => {
      ...
      return { zona, consumo, porcentaje: consumoSemanal > 0 ? (consumo / consumoSemanal) * 100 : 0 }
    })
  ...
  const getColorDias = (dias: number) => {
    if (dias > DIAS_AGUA_UMBRAL_ALTO) return { bg: 'bg-green-500', text: 'text-green-800', bgLight: 'bg-green-50' }
    if (dias >= DIAS_AGUA_UMBRAL_CRITICO) return { bg: 'bg-yellow-500', text: 'text-yellow-800', bgLight: 'bg-yellow-50' }
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className={`px-3 py-1 rounded-full text-white text-sm font-medium ${config.bg}`}>{config.label}</span>
      </div>
      ...
      <div>
        ...
        <div className="h-4 bg-gray-200 rounded-full overflow-hidden">
          <div className={`h-full ${config.bg} transition-all`} style={{ width: `${clamp(porcentaje, 0, 100)}%` }} />
        </div>
        <div className="text-center text-sm text-gray-500 mt-1">{porcentaje.toFixed(0)}% de capacidad</div>
      </div>
      ...
      <div className="space-y-3 pt-2">
          ...
          <div className={`${colorDias.bgLight} border border-${colorDias.bg.replace('bg-', '')} p-3 rounded`}>
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className={`text-2xl font-bold ${colorDias.text}`}>~{Math.floor(diasRestantes)} días</div>
              </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-cyan-50 p-3 rounded text-center">
            <div className="text-2xl font-bold text-cyan-600">{consumoSemanal.toFixed(2)}</div>
          ...
          <div className="bg-purple-50 p-3 rounded text-center">
            ...
            <div className="text-xs text-gray-500">{semanasRestantes === Infinity ? 'Sin consumo' : 'semanas'}</div>
          </div>
        </div>
      ...
      {configRecarga && diasHastaRecarga !== null && (
        <div className={`p-4 rounded-lg ${alcanzaHastaRecarga ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
          <div className="flex items-start justify-between">
            <div>
              <h3 className="text-sm font-bold text-gray-700 mb-1">Próxima Recarga</h3>
              <div className="text-lg font-bold text-gray-900">
                {format(new Date(configRecarga.proxima_recarga), "EEEE d 'de' MMMM", { locale: es })}
              </div>
              <div className="text-sm text-gray-600">En {diasHastaRecarga} días ({configRecarga.cantidad_litros.toLocaleString()} L)</div>
            </div>
            <div className={`text-2xl ${alcanzaHastaRecarga ? 'text-green-500' : 'text-red-500'}`}>
              {alcanzaHastaRecarga ? '✓' : '✗'}
            </div>
          </div>
          <div className={`mt-3 text-sm font-medium ${alcanzaHastaRecarga ? 'text-green-700' : 'text-red-700'}`}>
            {alcanzaHastaRecarga ? (
