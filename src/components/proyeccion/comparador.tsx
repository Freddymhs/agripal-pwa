...
export function Comparador({ zona, catalogoCultivos, costoAguaM3, suelo }: ComparadorProps) {
  const [cultivoA, setCultivoA] = useState<string>(catalogoCultivos[0]?.id || '')
  const [cultivoB, setCultivoB] = useState<string>(catalogoCultivos[1]?.id || '')
  ...
  return (
    <div className="space-y-3">
      <h5 className="text-xs font-medium text-gray-700">Comparar cultivos en {zona.nombre}</h5>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b">
                ...
                <th className="text-right py-1 text-gray-700 font-medium">{roiA.cultivo_nombre}</th>
                <th className="text-right py-1 text-gray-700 font-medium">{roiB.cultivo_nombre}</th>
              </tr>
            </thead>
            <tbody className="text-gray-600">
              <CompRow label="Plantas" a={roiA.num_plantas.toString()} b={roiB.num_plantas.toString()} />
              <CompRow label="Inversión" a={formatCLP(roiA.inversion_total)} b={formatCLP(roiB.inversion_total)} />
              <CompRow label="Agua/año" a={formatCLP(roiA.costo_agua_anual)} b={formatCLP(roiB.costo_agua_anual)} />
              <CompRow label="Ingreso Año 4" a={formatCLP(roiA.ingreso_año4)} b={formatCLP(roiB.ingreso_año4)} highlight />
              <CompRow
                label="ROI 4 años"
              ...
              <CompRow
                label="Equilibrio"
                a={roiA.punto_equilibrio_meses ? `${roiA.punto_equilibrio_meses}m` : 'N/A'}
                b={roiB.punto_equilibrio_meses ? `${roiB.punto_equilibrio_meses}m` : 'N/A'}
              />
            </tbody>
          </table>
...
function CompRow({
  label, a, b, highlight, colorA, colorB
}: {
  label: string; a: string; b: string; highlight?: boolean; colorA?: boolean; colorB?: boolean
}) {
  return (
    <tr className={highlight ? 'font-medium' : ''}>
      <td className="py-0.5 text-gray-500">{label}</td>
      <td className={`text-right py-0.5 ${colorA === true ? 'text-green-700' : colorA === false ? 'text-red-700' : ''}`}>{a}</td>
      <td className={`text-right py-0.5 ${colorB === true ? 'text-green-700' : colorB === false ? 'text-red-700' : ''}`}>{b}</td>
    </tr>
  )
}
