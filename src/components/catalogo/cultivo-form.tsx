...
export function CultivoForm({
  ...
  const [espaciadoMin, setEspaciadoMin] = useState(
    cultivo?.espaciado_min_m || 0.3,
  );
  ...
  const [tempMax, setTempMax] = useState<number | undefined>(
    cultivo?.clima?.temp_max_c,
  );
  const [toleranciaHeladas, setToleranciaHeladas] = useState<
    ToleranciaHeladas | undefined
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onGuardar({
      ...
      notas,
      clima:
        tempMin !== undefined || tempMax !== undefined || toleranciaHeladas
          ? {
              temp_min_c: tempMin,
              temp_max_c: tempMax,
  ...
  return (
    <form
      ...
      className="p-4 space-y-4 overflow-y-auto max-h-[calc(100vh-100px)]"
    >
