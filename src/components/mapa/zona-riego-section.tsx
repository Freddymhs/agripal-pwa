...
import { ESTADO_PLANTA, TIPO_RIEGO } from '@/lib/constants/entities'
...
interface ZonaRiegoSectionProps {
  ...
  onGuardarRiego: (zonaId: string, config: Zona['configuracion_riego']) => Promise<void>
}

export function ZonaRiegoSection({ zona, plantasVivas, catalogoCultivos, sueloArcilloso, onGuardarRiego }: ZonaRiegoSectionProps) {
  const [showConfigRiego, setShowConfigRiego] = useState(false)

  const consumoZonaM3Sem = calcularConsumoZona(zona, plantasVivas, catalogoCultivos)
  ...
  return (
    <>
      <div className={`p-3 rounded-lg ${zona.configuracion_riego ? 'bg-blue-50' : 'bg-amber-50/70'}`}>
        {zona.configuracion_riego ? (
          <div className="space-y-2">
            <div className="flex items-center gap-1 mb-1">
              <h4 className="text-sm font-bold text-blue-800">💧 Sistema de Riego</h4>
              <InfoLabel label="" tooltip="Estos valores usan tu caudal total (L/h) y horas de riego por día." />
            </div>
            <div className="bg-blue-100/60 border border-blue-200 rounded-lg p-2 space-y-0.5">
              ...
              <p className="text-[11px] text-blue-700">Consumo y días de cobertura se calculan con tu caudal y horas de riego.</p>
            </div>
            <div className="text-xs text-blue-700 bg-white p-2 rounded">
              <div className="flex justify-between">
                <span>Tipo:</span>
                <span className="font-medium">{zona.configuracion_riego.tipo === TIPO_RIEGO.PROGRAMADO ? '⏰ Programado' : '💧 Continuo 24/7'}</span>
              </div>
              <div className="flex justify-between">
                <span>Caudal:</span>
                <span className="font-medium">{zona.configuracion_riego.caudal_total_lh} L/h</span>
              </div>
              {zona.configuracion_riego.tipo === TIPO_RIEGO.PROGRAMADO && (
                <div className="flex justify-between">
                  <span>Horario:</span>
                  <span className="font-medium">{zona.configuracion_riego.horario_inicio} - {zona.configuracion_riego.horario_fin}</span>
                </div>
              )}
            </div>
            <button onClick={() => setShowConfigRiego(true)} className="w-full bg-blue-500 text-white py-2 rounded hover:bg-blue-600 text-sm font-medium">
              Reconfigurar Riego
            </button>
          </div>
        ) : (
          <div className="space-y-2">
            <div className="flex items-center gap-1 mb-1">
              ...
              <InfoLabel label="" tooltip='Mientras no configures caudal (L/h) y horas/día, la app usa un modelo estándar.' />
            </div>
            <div className="bg-amber-50 border border-amber-300 rounded-lg p-2.5 space-y-1">
              ...
              <p className="text-[11px] text-amber-700">El consumo y los días de cobertura se calculan solo con el consumo recomendado por ficha.</p>
            </div>
            <button onClick={() => setShowConfigRiego(true)} className="w-full bg-amber-500 text-white py-2 rounded hover:bg-amber-600 text-sm font-medium">
              Configurar Riego
            </button>
          </div>
        )}
      ...
      {showConfigRiego && (
        <ConfigurarRiegoModal
          ...
          onGuardar={async (config) => { await onGuardarRiego(zona.id, config); setShowConfigRiego(false) }}
