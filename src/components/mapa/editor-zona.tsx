...
interface EditorZonaProps {
  ...
  onRedimensionar: (nuevoTamaño: {
    ancho: number;
    alto: number;
  }) => Promise<{ error?: string }>;
  onMover: (nuevaPosicion: {
    x: number;
    y: number;
  }) => Promise<{ error?: string }>;
  onDelete: () => void;
...
export function EditorZona({
  ...
  const hayCambiosGeometricos =
    x !== zona.x || y !== zona.y || ancho !== zona.ancho || alto !== zona.alto;
  ...
  const handleSave = async () => {
    ...
    if (tipo !== zona.tipo && cantidadPlantas > 0) {
      setError(
        `No se puede cambiar el tipo: hay ${cantidadPlantas} planta(s) en esta zona`,
      );
      setSaving(false);
      return;
    }
  ...
  return (
    <ZoneEditorForm
      ...
      hayCambiosGeometricos={hayCambiosGeometricos}
