...
interface InteractionDeps {
  ...
  calcSnapGuides: (cursorMetros: Point) => { verticalX: number | null; horizontalY: number | null }
...
export function useMapInteractions(deps: InteractionDeps) {
  ...
  const { app, modoRef, propsRef, hitTestRef, overlayLayerRef, screenToWorld, viewport, calcSnapGuides, snapPosition } = deps
  ...
  useEffect(() => {
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      if (isMovingPlants && dragStartWorld) {
        handleMovingPlantsMove(worldPos, dragStartWorld, originalPlantaPositions, scale)
        return
      }
    ...
    const handleCrearZonaMove = (worldPos: Point, scale: number, drawing: boolean, start: Point | null) => {
      const metrosX = worldPos.x / PIXELS_POR_METRO
      const metrosY = worldPos.y / PIXELS_POR_METRO
      const clamped = {
        x: Math.max(0, Math.min(propsRef.current.terreno.ancho_m, metrosX)),
      ...
      if (drawing && start) {
        const snapped = snapPosition(clamped)
        const startPx = { x: start.x * PIXELS_POR_METRO, y: start.y * PIXELS_POR_METRO }
    ...
    const handlePlantasHover = (worldPos: Point, cvs: HTMLCanvasElement, scale: number) => {
  ...
  }, [app, screenToWorld, viewport, calcSnapGuides, snapPosition, modoRef, propsRef, hitTestRef, overlayLayerRef])
}
