...
export function PixiMapaTerrenoInner({
  ...
  const viewport = usePixiViewport(app, worldRef, {
    minScale: 0.01,
  ...
  const zoomTowardZona = useCallback(
    (direction: 1 | -1) => {
      ...
      const centerX =
        (zonaSeleccionada.x + zonaSeleccionada.ancho / 2) * PIXELS_POR_METRO;
      const centerY =
        (zonaSeleccionada.y + zonaSeleccionada.alto / 2) * PIXELS_POR_METRO;
      const canvasCenter = {
      ...
      if (worldRef.current) {
        worldRef.current.position.set(
          viewport.offsetRef.current.x,
          viewport.offsetRef.current.y,
        );
