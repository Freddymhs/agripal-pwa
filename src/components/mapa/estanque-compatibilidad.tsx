...
export function EstanqueCompatibilidad({
  compatibilidades,
}: EstanqueCompatibilidadProps) {
  if (compatibilidades.length === 0) return null;

  return (
    <div className="space-y-1.5">
      <h5 className="text-xs font-medium text-gray-700">
        Compatibilidad agua - cultivos
      </h5>
      {compatibilidades.map((c) => (
        <div
          key={c.cultivo_id}
          className={`p-2 rounded text-xs ${COLORES_COMPATIBILIDAD[c.nivel]}`}
        >
