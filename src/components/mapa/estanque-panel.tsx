...
function TooltipIcon({ text, variant = 'default' }: { text: string; variant?: 'default' | 'warning' | 'ok' }) {
  ...
  return (
    <div className="relative inline-block" onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)}>
      <button type="button" className={`w-3.5 h-3.5 rounded-full text-[10px] leading-none flex items-center justify-center ${colors}`} aria-label={text}>i</button>
      {show && (
        <div className="absolute right-0 top-4 z-40 max-w-xs bg-white border border-gray-200 rounded-lg shadow-lg p-2 text-[11px] text-gray-700 whitespace-normal break-words">{text}</div>
      )}
    </div>
  )
}
            ...
            <div className="grid grid-cols-3 gap-1 text-xs">
              {fuenteActual.boro_ppm != null && (
                <div className="bg-gray-50 p-1.5 rounded text-center">
                  ...
                  <div className="font-medium text-gray-900">{fuenteActual.boro_ppm} ppm</div>
                </div>
              )}
              {fuenteActual.salinidad_dS_m != null && (
                <div className="bg-gray-50 p-1.5 rounded text-center">
                  <div className="text-gray-500">Salinidad</div>
                  <div className="font-medium text-gray-900">{fuenteActual.salinidad_dS_m} dS/m</div>
                </div>
              )}
              {fuenteActual.ph != null && (
                <div className="bg-gray-50 p-1.5 rounded text-center">
                  <div className="text-gray-500">pH</div>
                  <div className="font-medium text-gray-900">{fuenteActual.ph}</div>
                </div>
              )}
            </div>
