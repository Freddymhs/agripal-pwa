...
export function RecomendacionViables({
  cultivos,
  seleccionados,
  areaHaFinal,
  onToggle,
}: RecomendacionViablesProps) {
        ...
        <div
          ...
          onClick={() => {
            const isSelected = seleccionados.some(
              (s) => s.cultivo.id === item.cultivo.id,
            );
            onToggle(item.cultivo, areaHaFinal, !isSelected);
          }}
        >
          ...
          <div className="text-xs text-gray-700 space-y-0.5">
            <div>
              💧 Agua: {item.cultivo.agua_m3_ha_año_min}-
              {item.cultivo.agua_m3_ha_año_max} m³/ha
            </div>
...
export function RecomendacionNoViables({
  cultivos,
}: RecomendacionNoViablesProps) {
  if (cultivos.length === 0)
    return (
      <div className="p-4 text-center text-gray-500 text-sm">
...
export function RecomendacionPlan({
  seleccionados,
  aguaDisponible,
  aguaSeleccionados,
  estadoAgua,
  onRemover,
}: RecomendacionPlanProps) {
    ...
    <div className="space-y-3">
      <div className="space-y-2 max-h-60 overflow-y-auto">
        {seleccionados.map((item) => (
          <div
            key={item.cultivo.id}
            className="p-2 bg-blue-50 rounded border border-blue-200 flex items-start justify-between"
          >
            <div className="text-sm text-blue-900 font-medium">
              {item.cultivo.nombre}
            </div>
            <button
              onClick={() => onRemover(item.cultivo, item.area_ha)}
      ...
      <div
        className={`p-3 rounded-lg border-l-4 ${estadoAgua.color === "red" ? "bg-red-50 border-red-500" : estadoAgua.color === "orange" ? "bg-orange-50 border-orange-500" : estadoAgua.color === "amber" ? "bg-amber-50 border-amber-500" : "bg-green-50 border-green-500"}`}
      >
        ...
        <div className="space-y-1 text-xs text-gray-700">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium">
              {aguaSeleccionados.agua_anual.toFixed(0)} m³
            </span>
          </div>
          <div className="flex justify-between">
            <span>Margen:</span>
            <span
              className={`font-medium ${porcentajeMargen >= 20 ? "text-green-700" : porcentajeMargen >= 10 ? "text-amber-700" : "text-red-700"}`}
            >
          ...
          <div className="flex justify-between border-t pt-1 mt-1">
            <span>Estado:</span>
            <span className={`font-bold text-${estadoAgua.color}-700`}>
              {estadoAgua.texto}
            </span>
          </div>
        </div>
