...
export function ChecklistLegal({
  legal,
  compact = false,
}: ChecklistLegalProps) {
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-2">
        <div
          className={`text-sm font-medium ${porcentaje === 100 ? "text-green-600" : porcentaje >= 50 ? "text-yellow-600" : "text-red-600"}`}
        >
          {porcentaje}% legal
        </div>
      ...
      {criticalMissing.length > 0 && (
        <div className="mb-3 p-2 bg-red-50 border border-red-200 rounded text-sm text-red-800">
          ...
          <span className="text-xs">
            Requerido para vender productos agrícolas
          </span>
        </div>
      )}
