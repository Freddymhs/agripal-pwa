    ...
    <div className="bg-white rounded-lg border shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-bold">
              Estado del Suelo: {estadoGeneral.texto}
            </h3>
            {!suelo?.quimico?.analisis_realizado && (
              <p className="text-sm opacity-75">
                Sin análisis de laboratorio verificado
              </p>
            )}
          </div>
        </div>
      ...
      <div className="p-4">
        ...
        <div className="space-y-3">
          {parametros.map((param) => {
            const porcentaje =
              param.valor !== undefined
                ? clamp((param.valor / param.max) * 100, 0, 100)
                : 0;
            ...
            return (
              <div key={param.nombre}>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span
                    className={`font-medium ${excede ? "text-red-600" : "text-gray-900"}`}
                  >
                    {param.valor !== undefined
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            {suelo.fisico.materia_organica_pct !== undefined && (
              <div>
                ...
                <span className="ml-2 font-medium text-gray-900">
                  {suelo.fisico.materia_organica_pct}%
                </span>
              </div>
            )}
          </div>
      ...
      {suelo?.quimico?.analisis_realizado && suelo.quimico.fecha_analisis && (
        <div className="p-3 bg-gray-50 text-xs text-gray-500 border-t">
          Análisis: {suelo.quimico.laboratorio || "Laboratorio"} -{" "}
          {suelo.quimico.fecha_analisis}
        </div>
      )}
    </div>
  );
