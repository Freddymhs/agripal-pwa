import type { FuenteAgua, CatalogoCultivo, CompatibilidadAguaCultivo, CompatibilidadNivel } from '@/types'
...
function evaluarCompatibilidadAgua(
  ...
  if (fuente.boro_ppm != null && fuente.boro_ppm > cultivo.boro_tolerancia_ppm) {
  ...
  if (fuente.salinidad_dS_m != null && fuente.salinidad_dS_m > cultivo.salinidad_tolerancia_dS_m) {
    const exceso = (fuente.salinidad_dS_m / cultivo.salinidad_tolerancia_dS_m).toFixed(1)
    problemas.push(
  ...
  if (problemas.length > 0) {
    const hayBoro = fuente.boro_ppm != null && fuente.boro_ppm > cultivo.boro_tolerancia_ppm
    const hayExcesoGrave = hayBoro && fuente.boro_ppm! > cultivo.boro_tolerancia_ppm * 2
    const haySalinidadGrave = fuente.salinidad_dS_m != null &&
