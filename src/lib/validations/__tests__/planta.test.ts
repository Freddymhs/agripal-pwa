...
const crearCultivoTest = (overrides?: Partial<CatalogoCultivo>): CatalogoCultivo => ({
  id: 'c1',
  proyecto_id: 'p1',
  nombre: 'Tomate',
  ...
  calendario: { meses_siembra: [1, 2, 3], meses_cosecha: [5, 6, 7], meses_descanso: [] },
  produccion: { produccion_kg_ha_año2: 50000, produccion_kg_ha_año3: 60000, produccion_kg_ha_año4: 60000, vida_util_dias: 120 },
...
describe('Validadores de Plantas', () => {
  ...
  describe('validarPosicionParaMover', () => {
    ...
    it('debe aceptar posiciones válidas dentro de la zona', () => {
      const resultado = validarPosicionParaMover({ x: 2, y: 2 }, zona, [], undefined)
      expect(resultado.valida).toBe(true)
    })

    it('debe rechazar posiciones fuera del límite X', () => {
      const resultado = validarPosicionParaMover({ x: -1, y: 5 }, zona, [], undefined)
      expect(resultado.valida).toBe(false)
      expect(resultado.error).toContain('Posición X')
    })

    it('debe rechazar posiciones fuera del límite X (máximo)', () => {
      const resultado = validarPosicionParaMover({ x: 11, y: 5 }, zona, [], undefined)
      expect(resultado.valida).toBe(false)
      expect(resultado.error).toContain('Posición X')
    })

    it('debe rechazar posiciones fuera del límite Y', () => {
      const resultado = validarPosicionParaMover({ x: 5, y: -1 }, zona, [], undefined)
      expect(resultado.valida).toBe(false)
      expect(resultado.error).toContain('Posición Y')
    })

    it('debe rechazar posiciones fuera del límite Y (máximo)', () => {
      const resultado = validarPosicionParaMover({ x: 5, y: 11 }, zona, [], undefined)
      expect(resultado.valida).toBe(false)
      expect(resultado.error).toContain('Posición Y')
    })
    ...
    it('debe validar espaciado mínimo cuando se proporciona cultivo', () => {
      ...
      const resultado = validarPosicionParaMover({ x: 5.5, y: 5 }, zona, plantasExistentes, cultivo)
      expect(resultado.valida).toBe(false)
    ...
    it('debe aceptar posiciones que respetan espaciado mínimo', () => {
      ...
      const resultado = validarPosicionParaMover({ x: 2, y: 2 }, zona, plantasExistentes, cultivo)
      expect(resultado.valida).toBe(true)
    })
    ...
    it('debe ignorar plantas de otras zonas para validación de espaciado', () => {
      ...
      const resultado = validarPosicionParaMover({ x: 5.5, y: 5 }, zona, plantasOtraZona, cultivo)
      expect(resultado.valida).toBe(true)
    })
  })
})
