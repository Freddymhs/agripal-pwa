...
export async function obtenerPendientes(): Promise<SyncItem[]> {
  ...
  return db.sync_queue
    ...
    .filter((item) => {
      if (item.estado === "error" && item.intentos >= MAX_RETRY_ATTEMPTS)
        return false;
...
export async function marcarError(id: UUID, error: string): Promise<void> {
  ...
  const baseDelay =
    RETRY_DELAYS[Math.min(intentos - 1, RETRY_DELAYS.length - 1)];
    ...
    if (item.datos_servidor) {
      ...
      if (tabla) {
        try {
          ...
          logger.error(
            "Error applying server data during conflict resolution",
            {
