...
async function ejecutarSyncInternal(
  adapter: SyncAdapter,
  result: SyncResult,
): Promise<SyncResult> {
  ...
  for (const entidad of SYNC_ENTIDADES) {
    try {
      ...
      for (const serverItem of response.data) {
        ...
        if (
          pendingQueueItem &&
          ["pendiente", "error", "sincronizando"].includes(
            pendingQueueItem.estado,
          )
        ) {
          continue;
        }
        ...
        const localRecord = localItem as {
          lastModified?: string;
          updated_at?: string;
        };
        const rawLocalTime = new Date(
          localRecord.lastModified || localRecord.updated_at || "",
        ).getTime();
      ...
      logger.error(`Error pulling ${entidad}`, {
