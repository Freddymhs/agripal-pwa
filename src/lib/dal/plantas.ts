...
export const plantasDAL = {
  ...
  add: (planta: Planta) => db.plantas.add(planta),

  bulkAdd: (plantas: Planta[]) => db.plantas.bulkAdd(plantas),

  update: (id: string, changes: Partial<Planta>) =>
    db.plantas.update(id, changes),

  delete: (id: string) => db.plantas.delete(id),

  bulkDelete: (ids: string[]) => db.plantas.bulkDelete(ids),
