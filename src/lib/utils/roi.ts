import type { CatalogoCultivo, Zona, FuenteAgua, Terreno, ConfiguracionRecarga, SueloTerreno } from '@/types'
...
export function obtenerCostoAguaM3(
  ...
  if (recargaConfig?.costo_recarga_clp && recargaConfig?.cantidad_litros && recargaConfig.cantidad_litros > 0) {
    return recargaConfig.costo_recarga_clp / (recargaConfig.cantidad_litros / 1000)
  }
  return 0
}
...
export function obtenerCostoAguaPromedio(
  ...
  for (const est of estanques) {
    ...
    const costo = obtenerCostoAguaM3(fuente, terreno, est.estanque_config?.recarga)
    if (costo > 0) costos.push(costo)
  }
  if (costos.length > 0) return costos.reduce((a, b) => a + b, 0) / costos.length
...
export function calcularROI(
  ...
  const precioPlanta = cultivo.precio_planta_clp ?? (cultivo.precio_kg_min_clp + cultivo.precio_kg_max_clp) / 2 * 0.5
  ...
  const aguaAnualTotal = consumoSemanalReal != null
  ...
  const kg2 = cultivo.produccion.produccion_kg_ha_año2 * factorArea * factorSuelo
  const kg3 = cultivo.produccion.produccion_kg_ha_año3 * factorArea * factorSuelo
  const kg4 = cultivo.produccion.produccion_kg_ha_año4 * factorArea * factorSuelo
  ...
  const roi4 = inversion > 0 ? ((ingresoAcumulado - inversion) / inversion) * 100 : 0
