...
function getTempActualMes(): number {
  ...
  const maxPorMes = [
    ...
    temps.maxima_verano_c - 8, temps.maxima_verano_c - 10,
    temps.maxima_verano_c - 10, temps.maxima_verano_c - 8,
    temps.maxima_verano_c - 5, temps.maxima_verano_c - 2,
    temps.maxima_verano_c, temps.maxima_verano_c,
  ]
  const minPorMes = [
    temps.minima_historica_c + 6, temps.minima_historica_c + 6,
    temps.minima_historica_c + 4, temps.minima_historica_c + 2,
    temps.minima_historica_c, temps.minima_historica_c,
    temps.minima_historica_c, temps.minima_historica_c,
    temps.minima_historica_c + 2, temps.minima_historica_c + 4,
    temps.minima_historica_c + 6, temps.minima_historica_c + 6,
  ]
...
export function evaluarRiesgoPlagas(
  ...
  return plagasCultivo.map(plaga => {
    let score = 0

    const temperaturaFavorable = plaga.temperatura_min != null && plaga.temperatura_max != null
      ? tempActual >= plaga.temperatura_min && tempActual <= plaga.temperatura_max
      : tempActual >= 15 && tempActual <= 35

    if (temperaturaFavorable) score += 50

    const etapaVulnerable = plaga.etapas_vulnerables?.includes(etapaActual) ?? false
