...
export const DURACION_ETAPAS: Record<string, Record<EtapaCrecimiento, number>> = {
  tomate: {
    'plántula': 30,
    'joven': 45,
    'adulta': 90,
...
function getDuracionEtapas(tipoCultivo: string): Record<EtapaCrecimiento, number> {
  const nombreNormalizado = tipoCultivo.toLowerCase().trim()

  for (const [cultivo, duraciones] of Object.entries(DURACION_ETAPAS)) {
    if (nombreNormalizado.includes(cultivo) || cultivo.includes(nombreNormalizado)) {
      return duraciones
    }
  }
