import type { EtapaCrecimiento } from '@/types'

export const KC_POR_CULTIVO: Record<string, Record<EtapaCrecimiento, number>> = {
  tomate: {
    'plántula': 0.45,
    'joven': 0.75,
    'adulta': 1.15,
    'madura': 0.85,
  },
  mango: {
    'plántula': 0.50,
    'joven': 0.80,
    'adulta': 1.10,
    'madura': 0.90,
  },
  zanahoria: {
    'plántula': 0.40,
    'joven': 0.70,
    'adulta': 1.00,
    'madura': 0.80,
  },
  papa: {
    'plántula': 0.45,
    'joven': 0.75,
    'adulta': 1.10,
    'madura': 0.75,
  },
  cebolla: {
    'plántula': 0.45,
    'joven': 0.70,
    'adulta': 1.05,
    'madura': 0.80,
  },
  ajo: {
    'plántula': 0.40,
    'joven': 0.70,
    'adulta': 1.00,
    'madura': 0.70,
  },
  lechuga: {
    'plántula': 0.45,
    'joven': 0.80,
    'adulta': 1.00,
    'madura': 0.90,
  },
  acelga: {
    'plántula': 0.40,
    'joven': 0.75,
    'adulta': 1.00,
    'madura': 0.85,
  },
  pimiento: {
    'plántula': 0.45,
    'joven': 0.75,
    'adulta': 1.10,
    'madura': 0.85,
  },
  ají: {
    'plántula': 0.45,
    'joven': 0.75,
    'adulta': 1.10,
    'madura': 0.85,
  },
  zapallo: {
    'plántula': 0.40,
    'joven': 0.70,
    'adulta': 1.00,
    'madura': 0.80,
  },
  melón: {
    'plántula': 0.45,
    'joven': 0.75,
    'adulta': 1.05,
    'madura': 0.75,
  },
  sandía: {
    'plántula': 0.45,
    'joven': 0.75,
    'adulta': 1.05,
    'madura': 0.70,
  },
  olivo: {
    'plántula': 0.50,
    'joven': 0.65,
    'adulta': 0.75,
    'madura': 0.65,
  },
  vid: {
    'plántula': 0.40,
    'joven': 0.60,
    'adulta': 0.80,
    'madura': 0.50,
  },
  uva: {
    'plántula': 0.40,
    'joven': 0.60,
    'adulta': 0.80,
    'madura': 0.50,
  },
  limón: {
    'plántula': 0.50,
    'joven': 0.70,
    'adulta': 0.85,
    'madura': 0.80,
  },
  limonero: {
    'plántula': 0.50,
    'joven': 0.70,
    'adulta': 0.85,
    'madura': 0.80,
  },
  naranjo: {
    'plántula': 0.50,
    'joven': 0.70,
    'adulta': 0.85,
    'madura': 0.80,
  },
  palto: {
    'plántula': 0.50,
    'joven': 0.75,
    'adulta': 0.95,
    'madura': 0.85,
  },
  guayabo: {
    'plántula': 0.50,
    'joven': 0.80,
    'adulta': 1.00,
    'madura': 0.85,
  },
  maíz: {
    'plántula': 0.40,
    'joven': 0.80,
    'adulta': 1.15,
    'madura': 0.70,
  },
  poroto: {
    'plántula': 0.35,
    'joven': 0.70,
    'adulta': 1.10,
    'madura': 0.35,
  },
  quinoa: {
    'plántula': 0.40,
    'joven': 0.70,
    'adulta': 1.00,
    'madura': 0.40,
  },
  alfalfa: {
    'plántula': 0.40,
    'joven': 0.80,
    'adulta': 1.20,
    'madura': 1.15,
  },
  oregano: {
    'plántula': 0.35,
    'joven': 0.60,
    'adulta': 0.90,
    'madura': 0.80,
  },
  orégano: {
    'plántula': 0.35,
    'joven': 0.60,
    'adulta': 0.90,
    'madura': 0.80,
  },
}

const KC_DEFAULT: Record<EtapaCrecimiento, number> = {
  'plántula': 0.45,
  'joven': 0.75,
  'adulta': 1.00,
  'madura': 0.85,
}

function quitarAcentos(str: string): string {
  return str.normalize('NFD').replace(/[\u0300-\u036f]/g, '')
}

export function getKc(tipoCultivo: string, etapa: EtapaCrecimiento): number {
  const nombreNormalizado = quitarAcentos(tipoCultivo.toLowerCase().trim())

  for (const [cultivo, kcs] of Object.entries(KC_POR_CULTIVO)) {
    const cultivoNormalizado = quitarAcentos(cultivo)
    if (nombreNormalizado.includes(cultivoNormalizado) || cultivoNormalizado.includes(nombreNormalizado)) {
      return kcs[etapa] ?? KC_DEFAULT[etapa]
    }
  }

  return KC_DEFAULT[etapa]
}

