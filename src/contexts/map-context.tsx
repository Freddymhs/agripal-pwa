...
export function MapProvider({ children }: { children: ReactNode }) {
  const { terrenoActual, zonas, plantas, catalogoCultivos, zonasHook, plantasHook, plantasLoteHook } = useProjectContext()
  ...
  useEffect(() => {
    const handleEscape = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && plantasSeleccionadas.length > 0) setPlantasSeleccionadas([])
    }
  ...
  useEffect(() => {
    if (zonaSeleccionada) {
      ...
      if (!zonaActualizada) { setZonaSeleccionada(null) }
      else if (JSON.stringify(zonaActualizada) !== JSON.stringify(zonaSeleccionada)) { setZonaSeleccionada(zonaActualizada) }
    }
  ...
  useEffect(() => {
    if (plantaSeleccionada) {
      ...
      if (!plantaActualizada) { setPlantaSeleccionada(null) }
      else if (JSON.stringify(plantaActualizada) !== JSON.stringify(plantaSeleccionada)) { setPlantaSeleccionada(plantaActualizada) }
    }
  }, [plantas, plantaSeleccionada])

  const plantasZonaSeleccionada = useMemo(() =>
    zonaSeleccionada ? plantas.filter(p => p.zona_id === zonaSeleccionada.id) : [],
    [plantas, zonaSeleccionada?.id]
  )

  const gridParams: GridParams | null = useMemo(() => {
    if (modo !== 'plantar' || !zonaSeleccionada || zonaSeleccionada.tipo !== TIPO_ZONA.CULTIVO) return null
  ...
  const posicionesOcupadas: Set<string> = useMemo(() => {
    ...
    for (const planta of plantasZonaSeleccionada) {
      set.add(`${Math.round((planta.x - margenX) / espaciado)},${Math.round((planta.y - margenY) / espaciado)}`)
    }
    return set
  }, [gridParams, zonaSeleccionada, plantasZonaSeleccionada])

  const getCultivoSeleccionado = useCallback(() => cultivoSeleccionado, [cultivoSeleccionado])
  ...
  const advertenciaEliminacionZona = useMemo(() =>
    zonaSeleccionada ? advertenciaEliminarZona(zonaSeleccionada, plantas) : null,
    [zonaSeleccionada, plantas]
  )

  const value: MapContextType = useMemo(() => ({
    modo, setModo, zonaSeleccionada, setZonaSeleccionada,
    plantaSeleccionada, setPlantaSeleccionada,
    plantasSeleccionadas, setPlantasSeleccionadas,
    rectNuevaZona, setRectNuevaZona,
    showGridModal, setShowGridModal,
    zonaPreview, setZonaPreview,
    cultivoSeleccionado, setCultivoSeleccionado,
    panelTab, setPanelTab,
    gridParams, posicionesOcupadas, plantasZonaSeleccionada,
    ...handlers,
    advertenciaEliminacionZona,
  }), [
    modo, zonaSeleccionada, plantaSeleccionada, plantasSeleccionadas,
    rectNuevaZona, showGridModal, zonaPreview, cultivoSeleccionado, panelTab,
    gridParams, posicionesOcupadas, plantasZonaSeleccionada,
    handlers, advertenciaEliminacionZona,
  ])
