...
import { terrenosDAL, zonasDAL, catalogoDAL } from '@/lib/dal'
import { obtenerCostoAguaPromedio } from '@/lib/utils/roi'
import { compararCultivos, type EscenarioCultivo } from '@/lib/utils/comparador-cultivos'
...
export default function EscenariosPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (terrenos.length > 0) {
        ...
        if (c.length >= 2) setSeleccion([c[0].id, c[1].id])
        else if (c.length === 1) setSeleccion([c[0].id])
      }
      setLoading(false)
    }
    fetchData()
  ...
  const escenarios = useMemo<EscenarioCultivo[]>(() => {
    ...
    return compararCultivos(cultivosSelec, zonaSeleccionada, terreno.suelo ?? null, costoAguaM3)
      ...
      <main className="p-4 space-y-4 max-w-4xl mx-auto">
        <div className="bg-indigo-50 border-l-4 border-indigo-500 p-4 rounded-lg">
          <p className="text-sm text-indigo-800">
            Compara hasta <strong>3 cultivos</strong> lado a lado para una zona. Evalúa ROI, costos, agua y compatibilidad con tu suelo.
          </p>
        </div>

        <div className="bg-white rounded-lg shadow p-4 space-y-3">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Zona a evaluar</label>
            <select
              value={zonaId}
            ...
            >
              ...
              {zonasCultivo.map(z => (
                <option key={z.id} value={z.id}>{z.nombre} ({z.area_m2} m²)</option>
              ))}
            </select>
          </div>
        ...
        {escenarios.length >= 2 && (
          <>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="text-left p-3 font-medium text-gray-700">Métrica</th>
                      {escenarios.map((e, i) => (
                        <th key={e.cultivo.id} className={`text-right p-3 font-medium ${COLORES_LINEA[i]}`}>
                          {e.cultivo.nombre}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    <tr>
                      ...
                      {escenarios.map((e, i) => (
                        <td key={i} className={`p-3 text-right font-bold ${e.roi.viable ? 'text-green-700' : 'text-red-700'}`}>
                    ...
                    <tr>
                      ...
                      {escenarios.map((e, i) => (
                        <td key={i} className="p-3 text-right text-gray-900">{formatCLP(e.roi.inversion_total)}</td>
                      ))}
                    </tr>
                    <tr>
                      <td className="p-3 text-gray-600">Ingreso Año 3</td>
                      {escenarios.map((e, i) => (
                        <td key={i} className="p-3 text-right text-green-700 font-medium">{formatCLP(e.roi.ingreso_año3)}</td>
                      ))}
                    </tr>
                    <tr>
                      <td className="p-3 text-gray-600">Costo/kg</td>
                      {escenarios.map((e, i) => (
                        <td key={i} className="p-3 text-right text-gray-900">{formatCLP(e.metricas.costoProduccionKg)}</td>
                      ))}
                    </tr>
                    <tr>
                      <td className="p-3 text-gray-600">Margen</td>
                      {escenarios.map((e, i) => (
                        <td key={i} className={`p-3 text-right font-medium ${
                          e.metricas.margenContribucion > 40 ? 'text-green-700' :
                          e.metricas.margenContribucion > 20 ? 'text-yellow-700' : 'text-red-700'
                        }`}>
                    ...
                    <tr>
                      ...
                      {escenarios.map((e, i) => (
                        <td key={i} className={`p-3 text-right font-medium ${
                          e.factorSuelo > 0.8 ? 'text-green-700' :
                          e.factorSuelo > 0.5 ? 'text-yellow-700' : 'text-red-700'
                        }`}>
                    ...
                    <tr>
                      ...
                      {escenarios.map((e, i) => (
                        <td key={i} className="p-3 text-right text-gray-900">
                          {e.roi.punto_equilibrio_meses != null ? `${e.roi.punto_equilibrio_meses} meses` : 'N/A'}
                        </td>
                      ))}
                    </tr>
                    <tr>
                      ...
                      {escenarios.map((e, i) => (
                        <td key={i} className="p-3 text-right text-gray-900">{e.roi.num_plantas}</td>
                      ))}
                    </tr>
                  </tbody>
                </table>
            ...
            <div className="bg-white rounded-lg shadow p-4">
              <h3 className="font-bold text-gray-900 mb-3">ROI Comparativo (Año 2 → 4)</h3>
                  ...
                  return (
                    <div key={e.cultivo.id}>
                      <div className="flex justify-between text-sm mb-1">
                        <span className={`font-medium ${COLORES_LINEA[i]}`}>{e.cultivo.nombre}</span>
                        <span className="text-gray-600">{formatCLP(e.roi.ingreso_acumulado_4años)}</span>
                      </div>
                      <div className="flex gap-1 h-6">
                        {[e.roi.ingreso_año2, e.roi.ingreso_año3, e.roi.ingreso_año4].map((ingreso, j) => (
                          <div
                            key={j}
                            className={`rounded ${COLORES_BG[i]} flex items-center justify-center text-xs ${COLORES_LINEA[i]}`}
                            style={{ width: `${maxIngreso > 0 ? (ingreso / maxIngreso) * 100 : 0}%`, minWidth: '20px' }}
                          >
                            A{j + 2}
                          </div>
                        ))}
            ...
            {(() => {
              const mejor = [...escenarios].sort((a, b) => b.roi.roi_4_años_pct - a.roi.roi_4_años_pct)[0]
              return (
                <div className="bg-green-50 border border-green-200 p-4 rounded-lg">
                  <h3 className="font-bold text-green-900 mb-1">Recomendación</h3>
                  <p className="text-sm text-green-800">
                    <strong>{mejor.cultivo.nombre}</strong> tiene el mejor ROI ({mejor.roi.roi_4_años_pct}%)
                    con margen de contribución del {Math.round(mejor.metricas.margenContribucion)}%
                    y factor de compatibilidad con suelo del {Math.round(mejor.factorSuelo * 100)}%.
                  </p>
                </div>
              )
            })()}
