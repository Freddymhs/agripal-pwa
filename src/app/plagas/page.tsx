  ...
  >(() => {
    ...
    for (const p of plantasZonaFiltradas) {
      if (!cultivosPorTipo.has(p.tipo_cultivo_id)) {
        ...
        if (cultivo) {
          cultivosPorTipo.set(p.tipo_cultivo_id, {
            cultivo,
      ...
      <main className="p-4 space-y-4 max-w-4xl mx-auto">
        <div className="bg-amber-50 border-l-4 border-amber-500 p-4 rounded-lg">
          <p className="text-sm text-amber-800">
            Evaluación de riesgo basada en <strong>temperatura actual</strong>,{" "}
            <strong>etapa del cultivo</strong> y{" "}
            <strong>severidad histórica</strong>.
          </p>
        </div>
        ...
        {riesgos.length === 0 && zonaId && (
          <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
            <p className="text-yellow-800 text-sm">
              No hay plantas activas en esta zona.
            </p>
          </div>
        )}
        ...
        {riesgos.map(({ cultivo, etapa, plagas }) => (
          <div key={cultivo.id} className="space-y-3">
            ...
            {plagas.length === 0 && (
              <div className="bg-green-50 border border-green-200 p-4 rounded-lg">
                <p className="text-green-800 text-sm">
                  No hay plagas registradas para este cultivo.
                </p>
              </div>
            )}
            ...
            {plagas.map((riesgo, i) => (
              <div
                key={i}
                className={`border rounded-lg p-4 ${ALERT_COLORS[riesgo.alertaNivel]}`}
              >
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <h3
                      className={`font-bold ${ALERT_TEXT[riesgo.alertaNivel]}`}
                    >
                      {riesgo.plaga.nombre}
                    </h3>
                ...
                <div className="grid grid-cols-3 gap-2 mb-3 text-xs">
                  <div
                    className={`p-2 rounded ${
                      riesgo.condicionesActuales.temperaturaFavorable
                  ...
                  <div
                    className={`p-2 rounded ${
                      riesgo.condicionesActuales.etapaVulnerable
                        ? "bg-red-100 text-red-800"
                        : "bg-green-100 text-green-800"
                    }`}
                  >
                  ...
                  {riesgo.plaga.severidad && (
                    <div
                      className={`p-2 rounded ${
                        riesgo.plaga.severidad === "critica" ||
                        riesgo.plaga.severidad === "alta"
                          ? "bg-red-100 text-red-800"
                ...
                {riesgo.plaga.medidas_preventivas &&
                  riesgo.plaga.medidas_preventivas.length > 0 && (
                    <div className="mb-2">
                ...
                <div className="text-xs text-gray-600 bg-white bg-opacity-50 p-2 rounded">
                  ...
                  {riesgo.plaga.control_recomendado || "No especificado"}
                </div>
              </div>
            ))}
          </div>
        ...
        {riesgos.length > 0 && totalAlertas === 0 && (
          <div className="bg-green-50 border border-green-200 p-4 rounded-lg text-center">
            <p className="text-green-800 font-medium">
              No hay riesgos altos detectados
            </p>
            <p className="text-green-700 text-sm mt-1">
              Las condiciones actuales son favorables para tus cultivos.
            </p>
          </div>
        )}
      </main>
