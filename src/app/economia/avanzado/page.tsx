...
import { calcularMetricasEconomicas, type MetricasEconomicas } from '@/lib/utils/economia-avanzada'
...
export default function EconomiaAvanzadaPage() {
  ...
  const global = useMemo(() => {
    ...
    const avgCostoKg = totalKg > 0
      ? resumen.reduce((s, r) => s + r.metricas.costoProduccionKg * r.metricas.kgProducidosAño, 0) / totalKg
      : 0
    const avgMargen = resumen.reduce((s, r) => s + r.metricas.margenContribucion, 0) / resumen.length
    const mejorRecuperacion = resumen
      ...
      <main className="p-4 space-y-4 max-w-4xl mx-auto">
        {global && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-white rounded-lg shadow p-4 text-center">
              <div className="text-3xl font-bold text-blue-700">{formatCLP(global.avgCostoKg)}</div>
            ...
            <div className="bg-white rounded-lg shadow p-4 text-center">
              ...
              <div className="text-xs text-purple-600 mt-1">Producción Anual (Año 3)</div>
            </div>
            <div className="bg-white rounded-lg shadow p-4 text-center">
              <div className={`text-3xl font-bold ${
                global.avgMargen > 40 ? 'text-green-700' : global.avgMargen > 20 ? 'text-yellow-700' : 'text-red-700'
              }`}>
                {Math.round(global.avgMargen)}%
              </div>
              <div className="text-xs text-gray-600 mt-1">Margen Contribucion</div>
              <div className={`text-xs mt-1 px-2 py-0.5 rounded-full inline-block ${
                global.avgMargen > 40 ? 'bg-green-100 text-green-800' :
                global.avgMargen > 20 ? 'bg-yellow-100 text-yellow-800' :
                'bg-red-100 text-red-800'
              }`}>
                {global.avgMargen > 40 ? 'Saludable' : global.avgMargen > 20 ? 'Ajustado' : 'Riesgo'}
              </div>
            </div>
            <div className="bg-white rounded-lg shadow p-4 text-center">
              <div className="text-3xl font-bold text-amber-700">
                {global.mejorRecuperacion != null ? `${global.mejorRecuperacion}m` : '-'}
              </div>
              <div className="text-xs text-amber-600 mt-1">Recuperacion Inversion</div>
            </div>
          </div>
        )}
        ...
        {resumen.length === 0 ? (
          <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
            <p className="text-yellow-800">
              No hay cultivos activos. Planta cultivos desde el mapa para ver métricas avanzadas.
            </p>
          </div>
        ) : (
              ...
              <div key={i} className="bg-white rounded-lg shadow p-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                  <div className="bg-blue-50 p-3 rounded">
                    ...
                    <div className="font-bold text-blue-800">{formatCLP(r.metricas.costoProduccionKg)}</div>
                    <div className="text-xs text-blue-500">vs venta {formatCLP(r.metricas.precioVentaKg)}</div>
                  </div>
                  <div className="bg-purple-50 p-3 rounded">
                    ...
                    <div className="text-xs text-purple-500">
                      de {Math.round(r.metricas.kgProducidosAño).toLocaleString('es-CL')} kg/año
                    </div>
                  </div>
                  <div className={`p-3 rounded ${
                    r.metricas.margenContribucion > 40 ? 'bg-green-50' :
                    r.metricas.margenContribucion > 20 ? 'bg-yellow-50' : 'bg-red-50'
                  }`}>
                    <div className="text-xs text-gray-600">Margen</div>
                    <div className={`font-bold ${
                      r.metricas.margenContribucion > 40 ? 'text-green-800' :
                      r.metricas.margenContribucion > 20 ? 'text-yellow-800' : 'text-red-800'
                    }`}>
                ...
                {r.metricas.kgProducidosAño > 0 && r.metricas.puntoEquilibrioKg != null && (
                  <div className="mt-3">
                    <div className="text-xs text-gray-500 mb-1">
                      Break-even: {Math.round(r.metricas.puntoEquilibrioKg / r.metricas.kgProducidosAño * 100)}% de producción
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full ${
                          r.metricas.puntoEquilibrioKg / r.metricas.kgProducidosAño < 0.6
        ...
        <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg text-sm">
          ...
          <ul className="text-blue-800 space-y-1 text-xs">
            <li><strong>Costo/kg:</strong> Costos operación anuales / kg producidos año 3</li>
            <li><strong>Punto equilibrio:</strong> Costos / (Precio venta - Costo variable)</li>
            <li><strong>Margen contribución:</strong> (Precio - Costo variable) / Precio x 100</li>
            <li><strong>Recuperación:</strong> Inversión total / Ingreso neto mensual</li>
          </ul>
        </div>
      </main>
    </div>
