...
export default function CatalogoPage() {
  ...
  const handleGuardar = async (data: Partial<CatalogoCultivo>) => {
    if (cultivoEditando) {
      ...
      await catalogoHook.agregarCultivo(data as Omit<CatalogoCultivo, 'id' | 'proyecto_id' | 'created_at' | 'updated_at'>)
    }
    ...
    >
      <div className="flex-1 flex overflow-hidden">
        <main className="flex-1 overflow-auto p-4 bg-gray-50">
          ...
          <div className="mt-8">
            <h2 className="text-lg font-bold text-gray-800 mb-3">Técnicas de Mejora</h2>
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
              {TECNICAS_MEJORA.map(t => (
                <div key={t.id} className="bg-white border rounded-lg p-3">
                  <div className="flex justify-between items-start mb-1">
                    <h3 className="font-medium text-sm text-gray-900">{t.nombre}</h3>
                    <span className="text-xs bg-green-100 text-green-700 px-1.5 py-0.5 rounded">{t.categoria}</span>
                  </div>
