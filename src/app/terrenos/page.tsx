...
interface TerrenoConConteo extends Terreno { zonasCount: number; plantasCount: number }
...
export default function TerrenosPage() {
  ...
  const cargarConteos = useCallback(async () => {
    ...
    const conConteo = await Promise.all(
      terrenos.map(async (terreno) => {
        ...
        const plantasCount = zonaIds.length > 0 ? await plantasDAL.countByZonaIds(zonaIds) : 0
  ...
  const handleCrearTerreno = async (data: { nombre: string; ancho_m: number; alto_m: number }) => {
    ...
    const nuevoTerreno = await crearTerreno({ proyecto_id: proyectoSeleccionado.id, ...data })
  ...
  const handleEliminarProyecto = async (proyecto: Proyecto) => {
    ...
    setEliminando({ tipo: 'proyecto', id: proyecto.id, nombre: proyecto.nombre, contenido })
  }

  const handleEliminarTerreno = async (terreno: Terreno) => {
    const contenido = await contarContenidoTerreno(terreno.id)
    setEliminando({ tipo: 'terreno', id: terreno.id, nombre: terreno.nombre, contenido })
  }
  ...
  const confirmarEliminacion = async () => {
    ...
    if (eliminando.tipo === 'proyecto') {
      ...
      if (proyectoSeleccionado?.id === eliminando.id) setProyectoSeleccionado(proyectos.find(p => p.id !== eliminando.id) ?? null)
    } else { await eliminarTerreno(eliminando.id) }
    setEliminando(null)
  }
  ...
  const handleGuardarProyecto = async () => {
    ...
    await editarProyecto(editandoProyecto.id, { nombre: editandoProyecto.nombre, ubicacion_referencia: editandoProyecto.ubicacion_referencia })
    setEditandoProyecto(null)
  }

  const handleGuardarTerreno = async () => {
    if (!editandoTerreno) return
    const result = await editarTerreno(editandoTerreno.id, { nombre: editandoTerreno.nombre, ancho_m: editandoTerreno.ancho_m, alto_m: editandoTerreno.alto_m })
    if (!result.error) { setEditandoTerreno(null); await refetchTerrenos() }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200 px-4 py-3">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/" className="text-gray-500 hover:text-gray-700">Volver al Mapa</Link>
            <h1 className="text-xl font-bold text-gray-900">Gestion de Terrenos</h1>
          </div>
        </div>
      </header>
      ...
      {showCrearProyecto && (
        <CrearProyectoModal onCreated={handleCrearProyecto} onCancel={() => setShowCrearProyecto(false)} />
      )}
