...
export default function PlanificadorAguaPage() {
  ...
  const { entradas } = useAgua(terreno, zonas, plantas, catalogoCultivos, refetch);
  ...
  const economiaAnual = useMemo(() => {
      ...
      .reduce(
        (acc, zona) => {
          ...
          const cultivosPorTipo = plantasZona.reduce(
            (grouped, p) => {
              grouped[p.tipo_cultivo_id] = (grouped[p.tipo_cultivo_id] || 0) + 1;
              return grouped;
            },
    ...
    return {
      ...
      roi: totals.inversionTotal > 0
        ? ((totals.ingresoTotal - totals.inversionTotal) / totals.inversionTotal) * 100
        : 0,
    };
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-blue-600 text-white px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link href="/economia" className="text-sm bg-emerald-600 px-3 py-1 rounded hover:bg-emerald-700">Economia</Link>
          <Link href="/agua" className="text-sm bg-cyan-600 px-3 py-1 rounded hover:bg-cyan-700">Gestion</Link>
        </div>
      </header>

      <main className="p-4 space-y-4 max-w-4xl mx-auto">
        <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-lg">
          <h2 className="text-lg font-bold text-blue-900 mb-2">Vista CEO: Proyeccion Anual</h2>
          <p className="text-sm text-blue-800">
            Simula y proyecta tu negocio agricola a <strong>12 meses</strong>.
            Anticipa problemas de agua, planifica recargas y calcula la economia.
          </p>
        </div>
        ...
        {tabActiva === "proyeccion" && proyeccion && (
          <PlanificadorProyeccionTab proyeccion={proyeccion} estanques={estanques} />
        )}

        {tabActiva === "economia" && economiaAnual && (
          <PlanificadorEconomiaTab economiaAnual={economiaAnual} plantas={plantas} />
        )}
        ...
        <div className="bg-green-50 border border-green-200 p-4 rounded-lg">
          ...
          <Link href="/" className="inline-block bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 text-sm font-medium">
            Ir al Mapa
          </Link>
        </div>
      </main>
