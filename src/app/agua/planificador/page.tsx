...
import { generarProyeccionAnual, type ProyeccionAnual, type EventoFuturo } from '@/lib/utils/agua-proyeccion-anual'
...
export default function PlanificadorAguaPage() {
  ...
  const [tabActiva, setTabActiva] = useState<'viabilidad' | 'proyeccion' | 'economia'>('viabilidad')
  ...
  const { estanques } = useEstanques(zonas, fetchData)
  const { entradas } = useAgua(terreno, zonas, plantas, catalogoCultivos, fetchData)
  ...
  const economiaAnual = useMemo(() => {
    ...
    return {
      ...
      roi: inversionTotal > 0 ? ((ingresoTotal - inversionTotal) / inversionTotal) * 100 : 0,
    }
  ...
  const maxNivel = proyeccion
    ? Math.max(...proyeccion.meses.map(m => Math.max(m.nivelInicio, m.nivelFin, m.recargas)))
    : 0
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-blue-600 text-white px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link href="/economia" className="text-sm bg-emerald-600 px-3 py-1 rounded hover:bg-emerald-700">
            💰 Economía
          </Link>
          <Link href="/agua" className="text-sm bg-cyan-600 px-3 py-1 rounded hover:bg-cyan-700">
            💧 Gestión
          </Link>
        </div>
      </header>
      ...
      <main className="p-4 space-y-4 max-w-4xl mx-auto">
        <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-lg">
          <h2 className="text-lg font-bold text-blue-900 mb-2">🔮 Vista CEO: Proyección Anual</h2>
          <p className="text-sm text-blue-800">
            Simula y proyecta tu negocio agrícola a <strong>12 meses</strong>.
            Anticipa problemas de agua, planifica recargas y calcula la economía.
          </p>
        </div>
        ...
        {tabActiva === 'proyeccion' && proyeccion && (
          <div className="space-y-4">
            <div className="bg-white rounded-lg shadow p-4">
              <h3 className="text-lg font-bold text-gray-900 mb-4">Nivel de Agua (12 meses)</h3>

              <div className="h-48 flex items-end gap-1">
                {proyeccion.meses.map((mes, i) => {
                  const alturaFin = maxNivel > 0 ? (mes.nivelFin / maxNivel) * 100 : 0
              ...
              <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                <div className="bg-cyan-50 p-3 rounded">
                  ...
                  <div className="font-bold text-cyan-800">{proyeccion.resumen.consumoTotalAnual.toFixed(1)} m³</div>
                </div>
                <div className="bg-blue-50 p-3 rounded">
                  <div className="text-xs text-blue-600">Recargas totales</div>
                  <div className="font-bold text-blue-800">{proyeccion.resumen.recargasTotales.toFixed(1)} m³</div>
                </div>
                <div className={`p-3 rounded ${proyeccion.resumen.mesesDeficit > 0 ? 'bg-red-50' : 'bg-green-50'}`}>
                  <div className={`text-xs ${proyeccion.resumen.mesesDeficit > 0 ? 'text-red-600' : 'text-green-600'}`}>
                    Meses déficit
                  </div>
                  <div className={`font-bold ${proyeccion.resumen.mesesDeficit > 0 ? 'text-red-800' : 'text-green-800'}`}>
                    {proyeccion.resumen.mesesDeficit}
                  </div>
                </div>
                <div className="bg-purple-50 p-3 rounded">
                  <div className="text-xs text-purple-600">Costo agua</div>
                  <div className="font-bold text-purple-800">{formatCLP(proyeccion.resumen.costosAgua)}</div>
                </div>
              </div>
              ...
              {!estanques.some(e => e.estanque_config?.recarga) && (
                <div className="mt-4 bg-yellow-50 border border-yellow-200 p-3 rounded text-sm text-yellow-800">
                  ⚠️ No tienes configurada la recarga. <Link href="/agua" className="underline font-medium">Configúrala aquí</Link> para proyectar mejor.
                </div>
              )}
            </div>

            <div className="bg-white rounded-lg shadow p-4">
              <h3 className="text-lg font-bold text-gray-900 mb-4">📅 Calendario de Eventos</h3>
              ...
              {proyeccion.eventos.length === 0 ? (
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {proyeccion.eventos.slice(0, 20).map((evento, i) => (
                    <div key={i} className={`flex items-center gap-3 p-2 rounded ${EVENTO_COLORS[evento.tipo]}`}>
        ...
        {tabActiva === 'economia' && economiaAnual && (
          <div className="bg-white rounded-lg shadow p-4">
            <h3 className="text-lg font-bold text-gray-900 mb-4">Economía Anual Proyectada</h3>

            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-green-50 p-4 rounded-lg text-center">
                <div className="text-3xl font-bold text-green-700">{formatCLP(economiaAnual.ingresoAnual)}</div>
                <div className="text-sm text-green-600">Ingresos año 2</div>
              </div>
              <div className="bg-blue-50 p-4 rounded-lg text-center">
                <div className="text-3xl font-bold text-blue-700">{formatCLP(economiaAnual.inversionTotal)}</div>
                <div className="text-sm text-blue-600">Inversión total</div>
              </div>
              <div className="bg-purple-50 p-4 rounded-lg text-center">
                <div className="text-3xl font-bold text-purple-700">{formatCLP(economiaAnual.costosAgua)}</div>
                <div className="text-sm text-purple-600">Costos agua/año</div>
              </div>
              <div className={`p-4 rounded-lg text-center ${economiaAnual.neto > 0 ? 'bg-emerald-50' : 'bg-red-50'}`}>
                <div className={`text-3xl font-bold ${economiaAnual.neto > 0 ? 'text-emerald-700' : 'text-red-700'}`}>
                  {formatCLP(economiaAnual.neto)}
                </div>
                <div className={`text-sm ${economiaAnual.neto > 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                  Neto anual
                </div>
              </div>
            </div>

            <div className={`p-4 rounded-lg text-center ${economiaAnual.roi > 50 ? 'bg-green-100' : economiaAnual.roi > 0 ? 'bg-yellow-100' : 'bg-red-100'}`}>
              <div className={`text-4xl font-bold ${economiaAnual.roi > 50 ? 'text-green-800' : economiaAnual.roi > 0 ? 'text-yellow-800' : 'text-red-800'}`}>
                {Math.round(economiaAnual.roi)}%
              </div>
              <div className={`text-sm ${economiaAnual.roi > 50 ? 'text-green-700' : economiaAnual.roi > 0 ? 'text-yellow-700' : 'text-red-700'}`}>
                ROI Proyectado
              </div>
            </div>
            ...
            {plantas.length === 0 && (
              <div className="mt-4 bg-yellow-50 border border-yellow-200 p-3 rounded text-sm text-yellow-800">
                No hay plantas. Agrega cultivos desde el mapa para ver proyecciones económicas.
              </div>
            )}
          </div>
        )}
        ...
        <div className="bg-green-50 border border-green-200 p-4 rounded-lg">
          <h3 className="text-sm font-bold text-green-900 mb-2">📋 Siguiente Paso</h3>
          <p className="text-xs text-green-800 mb-3">
            Una vez que hayas identificado cultivos viables, usa el <strong>mapa interactivo</strong> para plantar.
          </p>
          <Link
            href="/"
