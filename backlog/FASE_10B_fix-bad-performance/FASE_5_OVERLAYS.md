...
import { PIXELS_POR_METRO, COLOR_SELECCION, COLOR_SNAP, COLOR_PREVIEW_VALIDA, COLOR_PREVIEW_INVALIDA, COLOR_HOVER } from './pixi-constants'
...
export class PixiOverlayLayer {
  ...
  constructor() {
    ...
    this.dimensionText = new Text({ text: '', style: { fontSize: 14, fill: 0x1f2937, fontFamily: 'monospace' } })
  ...
  // ─────────────────────────────────────────────
  drawZonaPreview(
    preview: { x: number; y: number; ancho: number; alto: number; esValida: boolean },
    scale: number
  ): void {
    ...
    const color = preview.esValida ? COLOR_PREVIEW_VALIDA : COLOR_PREVIEW_INVALIDA
...
if (seleccionadasIds.length > 0 && clickedPlanta && seleccionadasIds.includes(clickedPlanta.id)) {
